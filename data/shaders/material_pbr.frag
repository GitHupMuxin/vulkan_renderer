...
layout (set = 0, binding = 5) uniform sampler2D samplerEu;
layout (set = 0, binding = 6) uniform sampler2D samplerEavg;
...
vec3 getIBLContribution(PBRInfo pbrInputs, vec3 n, vec3 reflection)
{
	...
	IBLContribution += diffuse + specular;
	if (uboParams.debugBSDFType == 1)
 	{ 	
		vec3 edgetint = vec3(0.827, 0.792, 0.678);
 		vec3 Favg = AverageFresnel(pbrInputs.specularColor, edgetint);
		float Eo = texture(samplerEu, vec2(pbrInputs.NdotV, sqrt(pbrInputs.alphaRoughness))).x;
		float Eavg = texture(samplerEavg, vec2(0.5, sqrt(pbrInputs.alphaRoughness))).x;
		vec3 fmsContribution = specularLight * Favg * Eavg * (1.0 - Eo) / (1 - Favg * (1 - Eavg));
		fmsContribution *= uboParams.scaleIBLAmbient;
		IBLContribution += fmsContribution;
	}
...
void main()
{
	...
	if (uboParams.debugBSDFType == 1)
	{
		vec3 edgetint = vec3(0.827, 0.792, 0.678);
 		vec3 Favg = AverageFresnel(pbrInputs.specularColor, edgetint);
		float Eo = texture(samplerEu, vec2(pbrInputs.NdotV, sqrt(pbrInputs.alphaRoughness))).x;
		float Ei = texture(samplerEu, vec2(pbrInputs.NdotL, sqrt(pbrInputs.alphaRoughness))).x;
		float Eavg = texture(samplerEavg, vec2(0.5, sqrt(pbrInputs.alphaRoughness))).x;
		vec3 fmsContribution = Favg * Eavg / (1 - Favg * (1 - Eavg)) * (1 - Eo) * (1 - Ei) / (M_PI * (1 - Eavg));
		specContrib += fmsContribution;
	}
