#include "engine/reference/VulkanApplication.h"
...
namespace engine::core
{
	...
	void VulkanApplication::renderNode(resource::vkglTF::Node *node, uint32_t cbIndex, resource::vkglTF::Material::AlphaMode alphaMode) {
		if (node->mesh) {
			// Render mesh primitives
			for (resource::vkglTF::Primitive * primitive : node->mesh->primitives) {
				if (primitive->material.alphaMode == alphaMode) {
					...
					if (alphaMode == resource::vkglTF::Material::ALPHAMODE_BLEND) {
	...
	void VulkanApplication::recordCommandBuffer()
	{
		...
		resource::vkglTF::Model &model = models.scene;
		...
		for (auto node : model.nodes) {
			renderNode(node, frameIndex, resource::vkglTF::Material::ALPHAMODE_OPAQUE);
		}
		// Alpha masked primitives
		for (auto node : model.nodes) {
			renderNode(node, frameIndex, resource::vkglTF::Material::ALPHAMODE_MASK);
		}
		// Transparent primitives
		// TODO: Correct depth sorting
		for (auto node : model.nodes) {
			renderNode(node, frameIndex, resource::vkglTF::Material::ALPHAMODE_BLEND);
		}
	...
	void VulkanApplication::createMaterialBuffer()
	{
		...
		for (auto& material : models.scene.materials) {
			...
			shaderMaterial.alphaMask = static_cast<float>(material.alphaMode == resource::vkglTF::Material::ALPHAMODE_MASK);
	...
	void VulkanApplication::setupDescriptors()
	{
		...
		*/
		...
		std::vector<resource::vkglTF::Model*> modellist = { &models.skybox, &models.scene };
		...
		*/
		...
		// Scene (matrices and environment maps)
		{
			...
			for (auto i = 0; i < descriptorSets.size(); i++) {
				...
				writeDescriptorSets[5].pImageInfo = &textures.Eu.descriptor;
				...
				writeDescriptorSets[6].pImageInfo = &textures.Eavg.descriptor;
	...
	void VulkanApplication::addPipelineSet(const std::string prefix, const std::string vertexShader, const std::string fragmentShader)
	{
		...
		VkVertexInputBindingDescription vertexInputBinding = { 0, sizeof(resource::vkglTF::Model::Vertex), VK_VERTEX_INPUT_RATE_VERTEX };
		std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
			{ 0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, pos)},
			{ 1, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, normal) },
			{ 2, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, uv0) },
			{ 3, 0, VK_FORMAT_R32G32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, uv1) },
			{ 4, 0, VK_FORMAT_R32G32B32A32_UINT, offsetof(resource::vkglTF::Model::Vertex, joint0) },
			{ 5, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, weight0) },
			{ 6, 0, VK_FORMAT_R32G32B32A32_SFLOAT, offsetof(resource::vkglTF::Model::Vertex, color) }
		};
	...
	*/
	...
	void VulkanApplication::generateE_uMap()
	{
		auto tStart = std::chrono::high_resolution_clock::now();
		...
		VK_CHECK_RESULT(vkCreateImage(device, &imageCI, nullptr, &textures.Eu.image));
		
		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(device, textures.Eu.image, &memReqs);
		VkMemoryAllocateInfo memAllocInfo{};
		memAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device, &memAllocInfo, nullptr, &textures.Eu.deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device, textures.Eu.image, textures.Eu.deviceMemory, 0));
		...
		viewCI.subresourceRange.levelCount = 1;
		viewCI.subresourceRange.layerCount = 1;
		viewCI.image = textures.Eu.image;
		VK_CHECK_RESULT(vkCreateImageView(device, &viewCI, nullptr, &textures.Eu.view));
		...
		samplerCI.maxAnisotropy = 1.0f;
		samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
		VK_CHECK_RESULT(vkCreateSampler(device, &samplerCI, nullptr, &textures.Eu.sampler));
		...
		framebufferCI.renderPass = renderPass;
		framebufferCI.attachmentCount = 1;
		framebufferCI.pAttachments = &textures.Eu.view;
		framebufferCI.width = dim;
		framebufferCI.height = dim;
		framebufferCI.layers = 1;
		...
		shaderStages = {
			loadShader(device, "genEuIS.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
			loadShader(device, "genEuIS.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
		};
		...
		//从显存里面传输 image 到 staging buffer
		VkCommandBuffer copyCmd = vulkanDevice->createCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);

		//改变 image layout to layout_transifer_src
		VkImageMemoryBarrier barrier{};
		barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		barrier.image = textures.Eu.image;
		...
		VkDeviceSize bufferSize = dim * dim * sizeof(float);
		Buffer staging;
		staging.device = device;
		vulkanDevice->createBuffer(
			VK_BUFFER_USAGE_TRANSFER_DST_BIT,
			VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
			bufferSize, &staging.buffer, &staging.memory
		);

		VkBufferImageCopy region{};
		region.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1};
		region.imageExtent = { (uint32_t)dim, (uint32_t)dim, 1 };
		vkCmdCopyImageToBuffer(copyCmd, textures.Eu.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, staging.buffer, 1, &region);

		// 转回 SHADER_READ_ONLY_OPTIMAL，之后 shader 才能采
		VkImageMemoryBarrier barrierBack{};
		barrierBack.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		barrierBack.image = textures.Eu.image;
		...
		// 用 gli 构造 texture2d 保存为 KTX
		gli::texture2d gliTex(
			gli::FORMAT_R32_SFLOAT_PACK32,
			gli::extent2d(
				static_cast<unsigned int>(dim),
				static_cast<unsigned int>(dim)));
		memcpy(gliTex.data(), staging.mapped, bufferSize);
		staging.unmap();
		staging.destroy();

		std::string ktxPath = assetpath + "Eu_map.ktx";
		if (gli::save_ktx(gliTex, ktxPath))
			std::cout << "Saved Eu map to " << ktxPath << std::endl;
		else
			std::cerr << "Failed to save Eu map!" << std::endl;
		...
		textures.Eu.descriptor.imageView = textures.Eu.view;
		textures.Eu.descriptor.sampler = textures.Eu.sampler;
		textures.Eu.descriptor.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textures.Eu.device = vulkanDevice;

		auto tEnd = std::chrono::high_resolution_clock::now();
		auto tDiff = std::chrono::duration<double, std::milli>(tEnd - tStart).count();
		std::cout << "Generating Eu map took " << tDiff << " ms" << std::endl;

	}

	void VulkanApplication::generateE_avgMap()
	{
		auto tStart = std::chrono::high_resolution_clock::now();
		...
		imageCI.tiling = VK_IMAGE_TILING_OPTIMAL;
		imageCI.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_SRC_BIT;
		VK_CHECK_RESULT(vkCreateImage(device, &imageCI, nullptr, &textures.Eavg.image));

		VkMemoryRequirements memReqs;
		vkGetImageMemoryRequirements(device, textures.Eavg.image, &memReqs);
		VkMemoryAllocateInfo memAllocInfo{};
		memAllocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = vulkanDevice->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		VK_CHECK_RESULT(vkAllocateMemory(device, &memAllocInfo, nullptr, &textures.Eavg.deviceMemory));
		VK_CHECK_RESULT(vkBindImageMemory(device, textures.Eavg.image, textures.Eavg.deviceMemory, 0));
		...
		viewCI.subresourceRange.levelCount = 1;
		viewCI.subresourceRange.layerCount = 1;
		viewCI.image = textures.Eavg.image;
		VK_CHECK_RESULT(vkCreateImageView(device, &viewCI, nullptr, &textures.Eavg.view));
		...
		samplerCI.maxAnisotropy = 1.0f;
		samplerCI.borderColor = VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE;
		VK_CHECK_RESULT(vkCreateSampler(device, &samplerCI, nullptr, &textures.Eavg.sampler));
		...
		framebufferCI.renderPass = renderPass;
		framebufferCI.attachmentCount = 1;
		framebufferCI.pAttachments = &textures.Eavg.view;
		framebufferCI.width = dim;
		framebufferCI.height = dim;
		framebufferCI.layers = 1;
		...
		writeDescriptorSets[0].pImageInfo = &textures.Eu.descriptor;

		vkUpdateDescriptorSets(device, static_cast<uint32_t>(1), writeDescriptorSets.data(), 0, nullptr);
		...
		shaderStages = {
			loadShader(device, "genEavg.vert.spv", VK_SHADER_STAGE_VERTEX_BIT),
			loadShader(device, "genEavg.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT)
		};
		...
		region.imageSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 };
		region.imageExtent = { (uint32_t)dim, (uint32_t)dim, 1 };
		vkCmdCopyImageToBuffer(copyCmd, textures.Eavg.image, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, staging.buffer, 1, &region);

		VkImageMemoryBarrier barrierBack{};
		barrierBack.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
		barrierBack.image = textures.Eavg.image;
		barrierBack.oldLayout = VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL;
		barrierBack.newLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		barrierBack.srcAccessMask = VK_ACCESS_TRANSFER_READ_BIT;
		barrierBack.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
		...
		std::string ktxPath = assetpath + "Eavg_map.ktx";
		if (gli::save_ktx(gliTex, ktxPath))
			std::cout << "Saved Eavg map to " << ktxPath << std::endl;
		else 
			std::cerr << "Failed to save Eavg map!" << std::endl;
		...
		textures.Eavg.descriptor.imageView = textures.Eavg.view;
		textures.Eavg.descriptor.sampler = textures.Eavg.sampler;
		textures.Eavg.descriptor.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
		textures.Eavg.device = vulkanDevice;


		auto tEnd = std::chrono::high_resolution_clock::now();
		auto tDiff = std::chrono::duration<double, std::milli>(tEnd - tStart).count();
		std::cout << "Generating Eavg map took " << tDiff << " ms" << std::endl;
	}
	...
	*/
	void VulkanApplication::generateCubemaps()
	{
		...
		for (uint32_t target = 0; target < PREFILTEREDENV + 1; target++) {
			...
			resource::VulkanTextureCubeMap cubemap;
	...
	*/
	...
	{
		// Scene
		shaderValusScene.projection = camera.matrices.perspective;
	...
	*/
	void VulkanApplication::updateOverlay()
	{
		...
		if (ui->header("Debug view")) {
			...
			if (ui->combo("BSDF", &debugBsdfType, bsdfType))
				shaderValuesParams.debugBsdfType = static_cast<float>(debugBsdfType);
